...
describe("перенесённые модули не разошлись с источником истины", () => {
  ...
  //
  // 🔴 «Тоже копии» — это утверждение о ЭТОМ каталоге, и оно должно остаться верным.
  // Фикстуры, снятые здесь и не имеющие истока в сабмит-репозитории, лежат в
  // подкаталогах (`fixtures/attestation/`), куда этот нерекурсивный readdirSync не
  // смотрит. Положенная рядом, такая фикстура красит сторожа в «в источнике нет
  // фикстуры» — и он прав, он не знает, что это не копия. Так и случилось 12.09: PR #22
  // положил `attestation-live.json` в этот каталог, и `main` стал красным. Не ослабляй
  // сторожа, положи в подкаталог.
