...
- 🔴 **Дымовая проверка утверждала СТАРУЮ форму — и не могла покраснеть.** Ветка успеха в
  ...
  ответом всегда, а не только при отказе. Проверено обоими путями: против
  недосягаемого хоста (как в CI) и против живого демо-шлюза.
...
⚠️ **Поправка к первой редакции этой записи.** Я написал, что в репозитории «не было CI» —
это неправда: `.github/workflows/ci.yml` живёт здесь с 04.09 (#17) и делает больше, чем я
собирался добавить (сборка, тесты, запуск собранного сервера по stdio, `npm pack
--dry-run`). Проверял локальным деревом, которое отстало на три коммита, вместо
`origin/main` — ровно та ошибка, за которую я правил чужие документы в тот же день. Свой
лишний воркфлоу снял; настоящая находка оказалась лучше выдуманной и стоит пунктом выше.
