...
- 🔴 **Дымовая проверка утверждала СТАРУЮ форму — и не могла покраснеть.** Ветка успеха в
  `scripts/smoke-test.ts` требовала `attestBody.pcr0_sha384` — то самое поле шлюза,
  которое пересылалось без проверки. В CI шлюз недосягаем **намеренно**, поэтому ветка
  успеха не выполнялась никогда: проверка стояла на том, что было сломано, и покраснеть
  не могла. Теперь она утверждает `verified === true`, наличие всех пяти исходов, PCR0 из
  подписанного документа, эхо нонса и то, что граница («чего это не доказывает») едет с
  ответом всегда, а не только при отказе. Проверено обоими путями: против недосягаемого
  хоста (как в CI) и против живого демо-шлюза.

Фальсификация (все четыре мутации роняют проверки, восстановление — 166/166): снять пин
корня; не отправлять нонс; сделать нонс постоянным; объявить подпись всегда верной.

⚠️ **Две поправки к первой редакции этой записи, обе мои.**

1. Я написал, что в репозитории «не было CI». Неправда: `.github/workflows/ci.yml` живёт
   здесь с 04.09 (#17) и делает больше, чем я собирался добавить — сборка, тесты, запуск
   собранного сервера по stdio, `npm pack --dry-run`. Проверял локальным деревом, которое
   отстало на три коммита, вместо `origin/main` — ровно та ошибка, за которую я в тот же
   день правил другие документы. Лишний воркфлоу снят.
2. Фикстура легла в `tests/fixtures/`, где сторож дрейфа из #19 утверждает, что **каждый**
   json там побайтово равен фикстуре сабмит-репозитория. Моя таковой не является, и после
   мержа #22 `main` стал красным одним тестом. Фикстура переехала в
   `tests/fixtures/attestation/`, сторож не ослаблен, а в его комментарии теперь сказано,
   почему подкаталог.
