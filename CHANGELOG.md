...
🔴 **Первый выпуск, в котором работа окна вообще выходит наружу.** В реестре с 02.09 лежит
`0.7.0`: шесть инструментов, слова «graph» внутри ноль. Весь путь чтения цены —
`get_verified_price`, `src/graph-price.ts`, перенесённые модули `src/graph/` — жил в
основной ветке и до пользователя не доезжал. Посторонний, разбиравший сабмит, поставил
пакет из реестра, не нашёл там чтения цены и написал, что его нет. **По факту он был
прав**, и это стоит держать в голове: опубликованный артефакт — единственное, что есть у
читателя.

### Added

- **Инструмент `get_verified_price`** (седьмой). Читает цену токена Uniswap V3 из The Graph
  и отдаёт её, только если проходят четыре проверки: индексатор подписал **ровно те байты**,
  которые пришли; этот подписант разрешается ончейн в индексатора со стейком; чтение —
  **пригодная** цена (подпись над GraphQL-ошибкой или над нулём ценой не является); и
  возраст самой цены измеряется **отдельно** от возраста головы сабграфа — голова бывает
  свежей, а цена в ней годовалой. При отказе цена не возвращается, а ответ называет и
  остановивший этап (`stage`), и причину этого этапа (`cause`).
- **Токен называть адресом, а не тикером.** Тикер в этом сабграфе не ключ: на `WETH`
  приходит несколько разных токенов с таким именем. Поэтому проверяются и возвращаются
  **все** совпадения, а не угадывается первое.
- Запрос стоит **один цент в USDC на Base** и требует `X402_PRIVATE_KEY`. Без ключа
  инструмент отказывает `no_payer_key` и **не тратит ничего**: неоплаченный запрос не
  отправляется, и подмены на непроверенный источник не происходит.

### Fixed

- 🔴 **README говорил «шесть инструментов», а сервер регистрировал семь**, и
  `get_verified_price` не был описан вообще. Публиковать инструмент, о котором в
  документации нет строки, — это отдать читателю пакет, который он не может прочитать.
  Список инструментов теперь **перечислен по именам**, а не посчитан: число в прозе
  устаревает в момент добавления, что здесь и произошло.
- `X402_PRIVATE_KEY` добавлен в таблицу переменных окружения — раньше платный путь требовал
  ключа, о котором документация молчала.

### Провенанс

Этот выпуск должен уйти в реестр с `npm publish --provenance`, и **из локальной машины это
невозможно** — не «неудобно», а невозможно. Замерено по коду установленного npm
(`libnpmpublish/lib/publish.js`): вне поддерживаемого CI бросается
`Automatic provenance generation not supported for provider: <имя>`, а в GitHub Actions без
прав — `Provenance generation in GitHub Actions requires "write" access to the "id-token"
permission`. Поэтому выпуск идёт отдельным воркфлоу с `id-token: write`; `--dry-run`
провенанс **не проверяет** и зелёным ничего не доказывает.

Сегодня реестр на запрос аттестаций отвечает `404`
(`/-/npm/v1/attestations/@usenami/signer-mcp@0.7.0`), и в метаданных версии ключа
`attestations` нет. Для продукта, который продаёт «проверь, что крутится», это та же дыра,
что и таблица измерений без подписи, только в npm.

## 0.7.1 — 2026-09-12

Один пункт, и он про то, что пакет обещал доказательство и не доказывал ничего.

### Fixed

- 🔴 **`get_attestation` шлёт нонс и проверяет документ.** Было: вызов `/attestation`
  без параметров, сырой JSON наружу и описание, обещающее, что инструмент «proves the
  code **currently** signing your orders matches the published source». Не держалась ни
  одна половина. Без нонса документ не привязан ни к чему — запись прошлогодней
  аттестации от такого клиента неотличима от свежей, а слово «currently» покупается
  именно нонсом. И не проверялось ничего: ни подпись железа, ни цепь сертификатов, ни
  корень. Инструмент пересылал блоб и утверждение.

  🔴 **Это единственная поверхность, которую потребляет чужой ИИ-агент** — то есть худшее
  место в продукте для украшения вместо проверяльщика. Найдено на третьем проходе
  постороннего по сабмиту; формулировка постороннего жёстче нашей: верификатор, не
  читающий подписанные байты, есть украшение.

  Теперь: свежие 16 байт нонса уходят в запрос, документ открывается, и возвращаются
  **пять названных исходов**, каждый из которых умеет покраснеть отдельно —
  `document_readable`, `nonce_echoed`, `root_pinned`, `chain_verified`,
  `signature_verified`. PCR0/PCR1/PCR2 читаются **из подписанных байтов**, а не из поля
  рядом. При любом красном документ всё равно отдаётся (на него можно смотреть), но
  `proves` пуст, а `do_not_trust_for` начинается с единственного честного чтения:
  документ, который не проверился, — это не слабое доказательство, а никакое.

  🔴 **`root_pinned` несёт всю нагрузку.** Тот, кто отвечает на `SIGNER_GATEWAY_URL`,
  выпускает свою CA под тем же subject `CN=aws.nitro-enclaves`, подписывает ею цепь и
  документ с любым измерением, отражает наш нонс — и остальные четыре проверки
  зеленеют. Замерено на НАСТОЯЩЕЙ подделке (своя CA, свой промежуточный, настоящая
  ES384 над настоящим `Sig_structure`, заявлено настоящее зарегистрированное
  измерение): ловит только сверка отпечатка корня с константой. Переменной окружения,
  ослабляющей пин, нет намеренно — агент, которого можно уговорить смягчить свой
  корень доверия, не имеет корня доверия. Как заменить нашу константу корнем,
  опубликованным AWS, напечатано в README и в выводе инструмента.

  Зависимостей не добавилось: CBOR разбирается своим декодером (~90 строк), цепь и
  подпись — штатным `node:crypto` (`X509Certificate`, `verify` с
  `dsaEncoding: "ieee-p1363"`, потому что COSE несёт подпись как сырые `r||s`, а не DER;
  с настройкой по умолчанию годный документ давал бы ложную красноту).

  Описание инструмента переписано: оно перечисляет пять проверок и **сразу** говорит,
  чего не устанавливает — соответствия PCR0 публичному исходнику (это отдельный шаг:
  пересборка образа или ончейн-реестр) и ничего о прошлых подписях.

- **Проверка, которая утверждала дефект, заменена на обратную.** В `tests/lib.test.ts`
  стоял тест «returns parsed JSON on 200»: он подавал `{ pcr0: "abc123" }` и требовал,
  чтобы `abc123` вышел наружу измерением. Именно это и было сломано. Теперь
  утверждается, что поле `pcr0` без подписанного документа за ним измерением НЕ
  становится.

- **Пример ответа в README больше не показывает `registered_onchain`.** Поля нет в живом
  ответе (замер 12.09: `attestation_doc_b64`, `pcr0_sha384`, `timestamp_ms`). Оно было
  настройкой оператора, поданной как факт о цепи.

Фальсификация (все четыре мутации роняют проверки, восстановление — 125/125): снять пин
корня; не отправлять нонс; сделать нонс постоянным; объявить подпись всегда верной.

## 0.7.0 — 2026-09-02

Этот релиз везёт две вещи, и вторая — долг: правка, которая лежала в `main`
непубликованной с 31 августа.

### Added
- **Hyperliquid в `place_order` и `cancel_order`** (mainnet и testnet). До этого пакет
  умел подписывать HL только через энклав, а сам маршрут не оборачивал — АЛГО
  упирались в это двое суток.

  Почему это не строка в `STRUCTURED_ORDER_VENUES`, а отдельный модуль: у HL
  подпись покрывает **объект действия**, а не HTTP-запрос, и адресуется он
  **целочисленным индексом актива**. Отсюда четыре шага, которых нет у HMAC-венью:
  резолв монеты в индекс из собственных метаданных площадки, формат размера и цены
  по `szDecimals`, сборка действия, отправка в `/exchange`.

  🔴 **Индекс никогда не угадывается.** Он читается из `meta` самой площадки, а
  неизвестная монета **отвергается**, а не подставляется: неверный индекс — это
  настоящий ордер на другом рынке, и подпись делает его подлинным.

  🔴 **Рыночный ордер отвергается, а не синтезируется.** У Hyperliquid нет типа
  «market»; площадка выражает его агрессивно оценённым немедленным лимитом, то есть
  выбором допустимого проскальзывания. Пакет не делает этот выбор за владельца денег
  и говорит, что послать вместо.

  Тест `submits EXACTLY the action it sent for signing` запирает главное: отправляется
  тот же объект, который подписывали. Расхождение означало бы верную подпись — под
  другим действием.

### Fixed
- 🔴 **Опубликованный 0.6.0 (11.08) сообщал `hyperliquid_main` как `denied`** —
  «MAINNET IS DENIED INSIDE THE ENCLAVE… supplying credentials will not change it».
  Этот статус ввели в 0.4.0 (06.08), описывая безусловный отказ энклава, который
  **ROT-1 снял днём раньше, 05.08**. Манифест уехал уже устаревшим, и 16.08 боевой
  ордер на Hyperliquid прошёл, пока пакет продолжал сообщать каждому агенту, что
  венью отвергается ещё до загрузки ключевого материала.

  Клейм безопасности, переживший свой механизм, хуже отсутствующего: читатель под
  него планирует. Починено в `#13` ещё 31.08, но релиз не резали — этот релиз и есть
  доставка той правки.
- Планка для `denied` поднята и записана в типе: не «сегодня отвергает», а
  безусловный отказ на уровне кода, который не снимается ни политикой, ни флагом, ни
  credential'ами. Сейчас его не несёт ничто.
- Оговорено, что значит `live`: «энклав подпишет, **если ключ провижионен**» — это НЕ
  утверждение о наличии ключа в вашем деплое, на такой вопрос отвечает шлюз, а не
  константа в пакете.
- Заметки манифеста про обе площадки Hyperliquid говорили «NOT reachable through this
  package's order tools». С этим релизом это неверно — переписаны, вместе с прямым
  указанием «limit only» и на чём это стоит.
